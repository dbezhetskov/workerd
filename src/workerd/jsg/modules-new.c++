 ...
 public:
  ...
  bool restoreEntry(
    ...
    KJ_IF_SOME(found, inner.lookup(innerContext)) {
      // Cache under the original (unstripped) specifier — the exact key runtime lookups
      // reproduce — while the baked module handle replaces the getDescriptor() compile.
